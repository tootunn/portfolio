...
function arlo_submenu_color(){
	"use strict";
	var list		= jQuery('.arlo_tm_submenu .submenu_inner .menu_wrap > ul > li');
	list.each(function(){
...
function arlo_tm_portfolio(){
	...
	if(jQuery().isotope) {
		...
		var list2 		 = jQuery('.arlo_tm_minimal_portfolio_list');
...
function robex_menu_bg(){
	...
	jQuery(window).on('scroll',function(){
		...
		var topMenuUniv		= jQuery('.arlo_tm_univ_topmenu');
		if(WinOffset >= 100){
			topBar.addClass('animate');
			topMenu.addClass('animate');
			topMenuUniv.addClass('animate');
		}else{
			topBar.removeClass('animate');
			topMenu.removeClass('animate');
			topMenuUniv.removeClass('animate');
		}
	});
}
